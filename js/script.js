...
const editUserRoleModalUserName = document.getElementById("edit-user-role-modal-user-name");
const editUserRoleModalSelect = document.getElementById("edit-user-role-select");
const editUserRoleModalCloseBtn = document.getElementById("edit-user-role-modal-close-btn");
const editUserRoleModalCancelBtn = document.getElementById("edit-user-role-modal-cancel-btn");
const editUserRoleModalConfirmBtn = document.getElementById("edit-user-role-modal-confirm-btn");
...
function addToCart(product) {
  ...
  if (existingItem) {
    ...
    if (product.stock > 0) {
      ...
      showNotification(`Estoque esgotado para ${product.description}`, "error");
      return;
    }
  }
  ...
  document.querySelectorAll(".cart-item-quantity").forEach((input) => {
    input.addEventListener("change", (e) => {
      ...
      if (newQuantity > maxStock) {
        showNotification(`Estoque insuficiente. Disponível: ${maxStock}`, "error");
...
function searchClients() {
  ...
  if (searchType === "all") {
    ...
    filteredClients = clients.filter(
      (client) => client.cnpj && client.cnpj.toLowerCase().includes(searchTerm)
    );
  }
...
function showConfirmationModal(itemName, onConfirm) {
    ...
    if (!modal || !modalItemName || !confirmBtn || !cancelBtn || !closeBtn) return;
    ...
    stockStatusChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: [stockStatusData.normal, stockStatusData.baixo, stockStatusData.esgotado],
                backgroundColor: [
